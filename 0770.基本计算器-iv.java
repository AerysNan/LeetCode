...
class Poly {
  ...
  Poly mul(Poly that) {
    ...
    for (List<String> k1 : this.count.keySet())
      for (List<String> k2 : that.count.keySet()) {
        ArrayList<String> kNew = new ArrayList<>();
        for (String x : k1)
          kNew.add(x);
        for (String x : k2)
          kNew.add(x);
  ...
  Poly evaluate(HashMap<String, Integer> evalMap) {
    Poly ans = new Poly();
    for (List<String> k : this.count.keySet()) {
  ...
  List<String> toList() {
    ...
    ArrayList<List<String>> keys = new ArrayList<>(this.count.keySet());
...
class Solution {
  ...
  public Poly parse(String expr) {
    ArrayList<Poly> bucket = new ArrayList<>();
